...
class SGFParser:
    ...
    ctrltrans           = str.maketrans("\000\001\002\003\004\005\006\007" +
    ...
    def _convertControlChars(self, text):
        """ Converts control characters in 'text' to spaces, using the
            ...
            behaviour."""
        return text.translate(self.ctrltrans)
...
class Collection(List):
    ...
    def __str__(self):
        ...
        return string.join(list(map(str, self.data)), "\n"*2)
    ...
    def __str__(self):
        ...
        if len(self):
            ...
            for n in map(str, self[1:]):
                ...
                s = s + n
                l = len(string.split(s, "\n")[-1])
            return s + string.join(list(map(str, [""] + self.variations)), "\n") + ")"
        else:
...
class Property(List):
    ...
    def __str__(self):
        return self.name + "[" + string.join(list(map(_escapeText, self)), "][") + "]"
...
class Cursor:
    ...
    def _setChildren(self):
        ...
        if self.index + 1 < len(self.gametree):
            ...
            self.children = [list[0] for list in self.gametree.variations]
